...
def _build_style_prompt(work_type: str, genre: Optional[str]) -> str:
    if work_type == "life":
        return (
            "作品類型：生活\n"
            "請使用貼近日常、真實自然、清楚流暢的語氣。"
            "避免過度堆砌意象或華麗辭藻，讓內容像可以直接分享的生活書寫。"
        )

    target_genre = genre or "散文"
    return (
        "作品類型：文學\n"
        f"指定文體：{target_genre}\n"
        "請確保回覆符合該文體的節奏、語感與表現方式，並保有文學性。"
    )
...
def _extract_json_object(raw: str) -> dict:
    ...
    raise AIServiceError("AI 回傳格式不正確")
...
async def get_writing_help(
    ...
    *,
    ...
    style_prompt = _build_style_prompt(work_type, genre)
    prompts = {
        "title": (
            "請根據以下內文，提供 3 個標題建議。\n"
            f"{style_prompt}\n"
            "請只輸出三個候選標題，每行一個，不要補充說明。\n\n"
            f"{context}"
        ),
        "polish": (
            "請幫我潤飾以下全文。\n"
            f"{style_prompt}\n"
            "請保留原意、核心事件與敘事重點，只優化語氣、節奏、遣詞與段落流暢度。"
            "請直接輸出潤飾後的完整內容，不要加前言或說明。\n\n"
            f"{context}"
        ),
    }
...
async def generate_draft_from_inspirations(
    ...
    *,
    ...
    prompt = f"""請根據以下靈感資料，為使用者創作一篇完整作品。
...
1. 必須產生一個合適的標題。
2. 必須產生可直接放入編輯器的完整內文。
3. 內容要忠於提供的靈感，不要加入與靈感無關的大量設定。
4. 若作品類型是生活，請寫得真實、自然、貼近日常。
5. 若作品類型是文學，請明確符合指定文體，並保有文學性。
...
"""
    ...
    raw = await call_ai(prompt, LITERARY_SYSTEM_PROMPT)
    data = _extract_json_object(raw)
    title = str(data.get("title", "")).strip()
    content = str(data.get("content", "")).strip()
    if not title or not content:
        raise AIServiceError("AI 產生的創作內容不完整")
